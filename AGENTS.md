...
Release is **tag-triggered and runs on CI** — never a local upload. That is what removes the old failure mode: a hand-built ZIP forgotten or misbuilt meant the auto-updater silently saw no new version, and users who noticed the tag and installed GitHub's auto-generated source ZIP ended up with a parallel `kntnt-gpx-blocks-<tag>/` plugin alongside the original. See `docs/updater.md` for the underlying mechanism.
...
The `release` job in `.github/workflows/ci.yml` fires only on a `v*` tag, and only after every gate (PHP 8.4 and 8.5, Node) is green. It verifies the tag against both version locations, runs `./build-release-zip.sh`, and **publishes** (not drafts) release `vX.Y.Z` with `kntnt-gpx-blocks.zip` attached and GitHub's generated notes as the body. A red gate leaves no release behind.

Pushing the tag is irreversible and live to users — don't push until the version is final. The `Updater` reads only the *latest published* release and identifies the asset by `content_type === "application/zip"`, not by filename, so the version-less filename is intentional.

**The `kntnt-code-skills:release` skill must NOT build the ZIP or run `gh release create` / `gh release edit` here** — CI owns build and publish, and a second publisher would race it. The skill's job ends at: bump the version everywhere, commit, tag `vX.Y.Z`, push branch + tag. `./build-release-zip.sh` is build-only and never uploads — run it locally only to inspect the package.
