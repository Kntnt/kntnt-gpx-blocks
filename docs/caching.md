...
- **GPX Statistics pattern** — each bound `core/paragraph` resolves through `Bindings\Statistics_Source::get_value()`, which calls `Attachment_Cache::get()` for the resolved attachment. The source's per-request memo (`"$post_id|$map_id"`) means the five binding-key calls per pattern instance share a single cache fetch.
