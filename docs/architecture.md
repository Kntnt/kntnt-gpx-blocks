...
The Statistics variation expands to plain `core/group` and `core/paragraph` blocks at insertion time, but the editor still needs a small amount of glue to surface meaningful preview values. By default the bindings system shows the source's `label` ("GPX statistics") for every bound paragraph whenever the resolved value is an empty string — and `Statistics_Source::get_value()` returns `''` from any error path — which would leave every row of the variation showing the same uninformative placeholder even after a Map is configured. To fix that, the plugin enqueues an editor-only script (`js/statistics-preview.js`, registered by `Bootstrap\Variation_Registrar`) that wraps `core/paragraph`'s edit component via an `editor.BlockEdit` HOC, fetches the resolved values from the editor-only REST endpoint `kntnt-gpx-blocks/v1/statistics-preview` (auth-gated to `edit_posts`, served by `Rest\Statistics_Preview_Controller`), and overrides `attributes.content` for the preview only — the override never reaches `setAttributes`, so the saved post content stays the empty string the bindings system writes. The endpoint runs the same `Resolve_Map_Id` + `Attachment_Cache` + `Value_Formatter` chain as the bindings source, so editor and front-end agree on which map answers and which numbers come back. Resolved values render in `var(--wp-block-synced-color)` — the purple Gutenberg uses for synced/bound attribute indicators — so the visual contract is self-evident. See [`blocks.md`](blocks.md) § *Editor preview* under *GPX Statistics variation*.
...
| `Rest\Statistics_Preview_Controller` | Editor-only REST endpoint (`kntnt-gpx-blocks/v1/statistics-preview`) that returns the formatted GPX statistics (distance, min/max elevation, ascent, descent) for a given `(postId, mapId)` pair. Consumed by the GPX Statistics editor preview HOC in `js/statistics-preview.js` to render real values in the bindings purple. Auth-gated to `edit_posts`. |
