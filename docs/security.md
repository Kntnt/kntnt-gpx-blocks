...
| Editor-supplied colour values on Map (`trackColor`, `trackCursorColor`, `waypointColor`, `tooltipBackground`, `tooltipNameColor`, `tooltipDescColor`) and Elevation (all seven attributes) | CSS custom property value | `Rendering\Color_Sanitizer::sanitize()` — accepts only three shapes: alpha-aware hex 3/4/6/8 (`#rgb`, `#rgba`, `#rrggbb`, `#rrggbbaa`), a strict single-arg CSS-variable reference `var(--ident)`, and the same reference with a single hex fallback `var(--ident, #hex)` (for the cases where `PanelColorSettings` emits an unresolved CSS custom property); rejects `rgb(...)`, `rgba(...)`, `hsl(...)`, named colours, nested `var()`, non-hex `var()` fallbacks, fallback chains, and any URL-injection attempt — then `esc_attr()` |
