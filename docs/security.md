...
| Editor-supplied colour values on Map (`trackColor`, `trackCursorColor`, `waypointColor`, `tooltipBackground`, `tooltipNameColor`, `tooltipDescColor`) and Elevation (all seven attributes) | CSS custom property value | `Rendering\Color_Sanitizer::sanitize()` — accepts only alpha-aware hex 3/4/6/8 (`#rgb`, `#rgba`, `#rrggbb`, `#rrggbbaa`); rejects `rgb(...)`, `rgba(...)`, `hsl(...)`, named colours, every shape of `var(...)` reference, and any URL-injection attempt — then `esc_attr()` |
