...
| Editor-supplied colour values on Elevation (all seven attributes) | CSS custom property value | `Rendering\Color_Sanitizer::sanitize()` — alpha-aware hex 3/4/6/8 whitelist plus a strict single-arg `var(--ident)` reference (with optional hex fallback) for the cases where `PanelColorSettings` emits an unresolved CSS custom property; rejects `rgb(...)`, `rgba(...)`, `hsl(...)`, named colours, nested `var()`, non-hex `var()` fallbacks, fallback chains, and any URL-injection attempt — then `esc_attr()` |
