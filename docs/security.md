...
| Editor-supplied colour values on Map (`trackColor`, `trackCursorColor`, `waypointColor`, `tooltipBackground`, `tooltipNameColor`, `tooltipDescColor`) and Elevation (`axisColor`, `axisLabelColor`, `lineColor`, `cursorColor`, `tooltipBackground`, `tooltipColor`) | CSS custom property value | `Rendering\Color_Sanitizer::sanitize()` — accepts only alpha-aware hex 3/4/6/8 (`#rgb`, `#rgba`, `#rrggbb`, `#rrggbbaa`); rejects `rgb(...)`, `rgba(...)`, `hsl(...)`, named colours, every shape of `var(...)` reference, and any URL-injection attempt — then `esc_attr()` |
| Editor-supplied font references on Map (waypoint tooltip name/desc) | CSS custom property value | Whitelist regex (matches CSS variable references and a handful of keywords); fallback to default if outside the whitelist. Elevation no longer carries dedicated typography attributes — typography is delegated to core's block-level `supports.typography`, which sanitises the saved `style.typography.*` slot through `wp_kses_post()` on the wrapper's inline `style` attribute. |
