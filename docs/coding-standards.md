...
This section pins the placeholders and policy choices used elsewhere
in this document to the concrete values that apply to the
`kntnt-gpx-blocks` plugin. Where a rule above says
"`kntnt-<project>`" or "latest stable", read it as the value below.

### Identifiers

| Where | Value |
|---|---|
| Plugin slug, repo name, directory name, text domain | `kntnt-gpx-blocks` |
| Plugin main file | `kntnt-gpx-blocks.php` |
| PHP namespace root | `Kntnt\Gpx_Blocks` |
| PSR-4 source directory | `classes/` (WordPress override of the PHP default `src/`) |
| WordPress hook / filter / option / meta-key prefix | `kntnt_gpx_blocks_` |
| CSS class prefix | `kntnt-gpx-blocks-` (no abbreviation) |
| REST namespace | `kntnt-gpx-blocks/v1` |
| Block namespace (in `block.json` `name` field) | `kntnt-gpx-blocks/<slug>` |
...
| TypeScript module system | ESM |

### Tooling

The project uses the standard tooling described in the relevant
modules above, with no substitutions. Concretely:

- **PHP:** Composer for autoloading and dev dependencies; Pest +
  Brain Monkey + Mockery for unit tests; PHPStan max with
  `szepeviktor/phpstan-wordpress`; PHPCS with the WordPress Coding
  Standards plus the four deliberate deviations documented in the
  WordPress section.
- **Blocks:** `@wordpress/scripts` for build, lint, format, and
  Jest-based unit tests. No Biome, no Bun, no Vitest in the block
  pipeline.
- **Integration tests:** WordPress Playground via
  `@wp-playground/cli`. DDEV is not used; the plugin's behaviour
  doesn't require a real MySQL.
- **End-to-end tests** (when added): Playwright +
  `@wordpress/e2e-test-utils-playwright`.

### Distribution

- `build/` is committed to git so users who clone don't need a Node
  toolchain.
- `vendor/` is git-ignored. Releases are produced by a build script
  that runs `composer install --no-dev --optimize-autoloader` and
  zips the result.
