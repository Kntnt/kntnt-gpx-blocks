...
| `enablePan` | boolean | `true` | The **Pan** result — drag (mouse and single-touch), trackpad two-finger swipe, and the four arrow keys. Gates `map.dragging`, the wheel handler's `'pan'` branch, and the arrow keys via a capture-phase key filter on the map container. |
| `enableZoom` | boolean | `true` | The **Zoom** result — Cmd/Ctrl + wheel, pinch (touchscreen via `map.touchZoom` and trackpad via the wheel handler), double-click, and the `+` / `-` / `=` keys. Gates `map.touchZoom`, `map.doubleClickZoom`, the wheel handler's `'zoom'` branch (both Cmd/Ctrl + wheel on a mouse and the trackpad-pinch gesture browsers deliver as a wheel event with `ctrlKey: true`), the `+` / `-` / `=` keys via the same capture-phase key filter, and the wheel handler's hint-overlay suppression — when off, the map is fully passive to wheel events: no zoom and no modifier-key hint overlay; the page scrolls past as if the map were a static image. The Pan branch is independent and still obeys `enablePan`. |
...
3. **Interactions** — three result-named toggles, in this order: **Pan**, **Zoom**, **Track cursor**. The first two gather every Leaflet mechanism behind the visitor-facing result the editor wants to enable or disable; *Track cursor* is the existing per-block `enableTrackPositionCursor` toggle, unchanged. Box zoom is not a toggle and is removed from the wheel handler. *Pan* covers drag (mouse and single-touch via `map.dragging`), trackpad two-finger swipe (the wheel handler's `'pan'` branch), and the four arrow keys (via a capture-phase key filter on the map container that swallows them before Leaflet's bubble-phase handler runs). *Zoom* covers Cmd/Ctrl + wheel, pinch (touchscreen via `map.touchZoom` and trackpad pinch via the wheel handler's `'zoom'` branch — browsers deliver trackpad pinch as a wheel event with `ctrlKey: true`), double-click (`map.doubleClickZoom`), and the `+` / `-` / `=` keys (via the same capture-phase key filter). When *Zoom* is off, the wheel handler also suppresses the modifier-key hint overlay because the "Hold ⌘ + scroll to zoom the map" message is misleading when zoom is disabled altogether; the *Pan* branch is independent and still surfaces the overlay only when *Pan* is off and *Zoom* is on. The keyboard handler itself is enabled whenever either result is on so the map remains focusable for the at-least-one-enabled key group.
4. **Waypoint info** — two toggles, `Name` and `Description`, in line with WordPress core's convention that a `ToggleControl` label names the thing being toggled (the verb is implicit in the widget). Both default on. They control whether the corresponding line is rendered inside the per-marker tooltip; setting both off suppresses the tooltip entirely (no hover surface, no sticky-on-click). When a toggle is off, the dependent controls in the Styles tab hide rather than stay visible-but-inert (issue #143): turning `Name` off omits the `Waypoint name` row in the Color panel and the `Waypoint name` typography PanelBody; turning `Description` off omits the corresponding `Waypoint description` surfaces; turning *both* off additionally omits the shared `Waypoint background` row (it has no remaining content to back). Hidden attribute values are not cleared on hide — re-enabling the master toggle restores the controls with their saved values. The same symmetry applies in the Settings tab to the `Cursor` row, which hides when `Track cursor` (in *Interactions*) is off.

**Styles tab**, in order:

1. **Color** — a single `PanelColorSettings` with `enableAlpha: true` exposing up to six entries in this order: **Track** (`trackColor`), **Cursor** (`trackCursorColor`), **Marker** (`waypointColor`), **Waypoint background** (`tooltipBackground`, alpha-bearing 8-digit hex; also drives the tooltip arrow tip so a semi-transparent background produces a semi-transparent arrow), **Waypoint name** (`tooltipNameColor`), **Waypoint description** (`tooltipDescColor`). Every colour defaults to empty; an unset value falls back to the hardcoded SCSS default. Rows whose master toggle is off are omitted from the panel rather than rendered visible-but-inert (issue #143): `Cursor` follows `Track cursor` (in *Interactions*), `Waypoint name` follows `Name` (in *Waypoint info*), `Waypoint description` follows `Description`, and the shared `Waypoint background` is omitted only when *both* tooltip content toggles are off. The saved attribute value is not cleared on hide.
2. **Waypoint name** — a unified Typography `ToolsPanel` (the same `__experimentalToolsPanel` + `__experimentalToolsPanelItem` pattern core Paragraph and Group use) covering all seven aspects WordPress's standard typography surface offers: **Font** (`FontFamilyControl`), **Size** (`FontSizePicker`), **Appearance** (`FontAppearanceControl` — combined weight + style), **Line height** (`LineHeightControl`), **Letter spacing** (`LetterSpacingControl`), **Decoration** (`TextDecorationControl`), and **Letter case** (`TextTransformControl`). Each aspect can be enabled or disabled individually; an unset aspect inherits from the theme. "Reset all" clears every aspect. The whole PanelBody is omitted when the `Name` toggle in *Waypoint info* is off (issue #143); the saved attributes are not cleared, so re-enabling the toggle restores the panel with its previous values.
3. **Waypoint description** — same surface as *Waypoint name*, mapped to the `tooltipDesc*` attribute family. The whole PanelBody is omitted when the `Description` toggle is off, with the same persistence guarantee.
...
The block toolbar also exposes the auto-generated `mapId` as a click-to-copy badge (issue #147). A second `<BlockControls>` with **no** `group` prop sits between the alignment group and the `other`-group Replace flow and renders a `<ToolbarButton>` whose visible text is the literal `mapId` value (e.g. `map-7k9f2m`) — byte-identical to what gets copied. The visual decoration (monospace font, subtle background pill, rounded corners, 4 px horizontal margin clearing the toolbar group dividers) is editor-only and lives in `src/blocks/map/editor.scss` under the `.kntnt-gpx-blocks-map-id-badge` selector; no `#` or other prefix surfaces in the text itself. Clicking the badge calls `navigator.clipboard.writeText( mapId )` and dispatches a success snackbar (`Map ID copied`) through `core/notices`; a rejected clipboard promise surfaces an error snackbar (`Couldn't copy Map ID`) instead, with no `document.execCommand` fallback since modern WP admin runs over HTTPS where `navigator.clipboard` is reliably available. The visual tooltip is `Copy Map ID` (verb-first, matching toolbar convention) while the `aria-label` is built via `sprintf( __( 'Copy Map ID %s to clipboard' ), mapId )` so screen-reader users hear *which* id will be copied before activating the button. The badge renders only when `mapId` is non-empty (after `useEnsureUniqueMapId` has assigned a value), so a freshly inserted block never flashes an empty pill.
...
`supports.dimensions` enables `aspectRatio` and `minHeight`, which surface the standard core Dimensions panel for sizing. The chosen values land under `attributes.style.dimensions.*` like every other dimensions-aware core block and reach the wrapper as plain inline `aspect-ratio` / `min-height` styles via `get_block_wrapper_attributes()`. An empty Minimum height field falls back to the SCSS baseline (`min-height: 30vh;`); any user-set `aspect-ratio` stacks alongside via the normal CSS cascade. The `Rendering\Dimensions_Defaults` filter (issue #117, simplified in #146) writes `style.dimensions.minHeight = '30vh'` onto the parsed block's `attrs` whenever `minHeight` is blank, regardless of `aspectRatio` — see the *Sizing* paragraph above for the full mechanism.
...
8. Attaches the wheel handler on the block element: `Cmd`/`Ctrl`+wheel and trackpad pinch (delivered as a wheel event with `ctrlKey:true`) zoom around the cursor *only when `enableZoom` is true* — when Zoom is disabled in the sidebar, every wheel event falls through to the hint path and the modifier-key overlay is suppressed too (the "Hold ⌘ + scroll to zoom the map" message is misleading when zoom is off altogether), so the map behaves as a passive static image for wheel input (issue #139); trackpad two-finger pan (wheel with `deltaMode === 0` and no modifier) pans the map *only when `enablePan` is true* — when Pan is disabled in the sidebar, the gesture falls through to the hint path so the page scrolls and the modifier-key overlay surfaces (the Pan toggle is honoured across all input modalities, not just mouse and single-touch drag), unless Zoom is also off, in which case the overlay is suppressed for the same reason; a regular mouse wheel (`deltaMode === 1+` and no modifier) does not move the map but surfaces a brief overlay reminding the user to hold the modifier to zoom (suppressed when Zoom is off), and the page scrolls past the map normally. The wheel handler is paired with a capture-phase `keydown` filter on the block element that swallows the arrow keys when `enablePan === false` and the `+` / `-` / `=` keys when `enableZoom === false`, splitting Leaflet's monolithic keyboard handler into the two result-named groups the Inspector surfaces.
...
The Settings tab additionally carries a **Cursor & guides** PanelBody (issue #144) directly above the Tooltip info panel, with three toggles in order: `Cursor` (master, default on), `Vertical guide` (default on), `Horizontal guide` (default off). The two guide sub-toggles are hidden from the panel whenever the master `Cursor` toggle is off and reappear with their saved values when the master is re-enabled (same hide-on-master-off pattern as issue #143). Turning `Cursor` off is a full functional disablement on the Elevation side: the cursor `<g>` group, hit-rect, dot, and any guide `<line>`s never enter the SVG; pointer handlers are not bound (Elevation does not produce `fraction` writes); the `onElevationCursorChange` watch returns silently. Each guide's `<line>` is created only when its respective toggle is on; the dot and hit-rect always exist when the cursor is on. The Map block's own `enableTrackPositionCursor` is unaffected by these settings.

The Settings tab additionally carries a **Tooltip info** PanelBody with two toggles, `Distance` and `Height` (in line with WordPress core's convention that a `ToggleControl` label names the thing being toggled — the verb is implicit in the widget). Both default on. When a toggle is off, the dependent controls in the Styles tab hide rather than stay visible-but-inert (issue #143): turning `Distance` off omits the `Tooltip distance` row in the Color panel and the `Tooltip distance` typography PanelBody; turning `Height` off omits the corresponding `Tooltip height` surfaces; turning *both* off additionally omits the shared `Tooltip background` row. The `Cursor` row in the Color panel hides whenever `showCursor` is off (issue #144), using the same symmetry pattern. Hidden attribute values are not cleared on hide — re-enabling the master toggle restores the controls with their saved values.

**Styles tab** (rendered through a second `<InspectorControls group="styles">` slot), in order:

1. **Color** — `PanelColorSettings` for the nine colours (`Background`, `Plot line`, `Plot fill`, `Cursor`, `Axis`, `Axis labels`, `Tooltip background`, `Tooltip distance`, `Tooltip height`), filtered per the symmetry rule above.
