...
The Settings tab additionally carries a **Tooltip info** PanelBody with two toggles, `Distance` and `Height` (in line with WordPress core's convention that a `ToggleControl` label names the thing being toggled — the verb is implicit in the widget). Both default on. When a toggle is off, the dependent controls in the Styles tab hide rather than stay visible-but-inert (issue #143): turning `Distance` off omits the `Tooltip distance` row in the Color panel and the `Tooltip distance` typography PanelBody; turning `Height` off omits the corresponding `Tooltip height` surfaces; turning *both* off additionally omits the shared `Tooltip background` row. Hidden attribute values are not cleared on hide — re-enabling the master toggle restores the controls with their saved values.
