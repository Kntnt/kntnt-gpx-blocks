...
- **Persistence:** every attribute that is a colour or a font reference stores whatever the WordPress editor component delivers — for colours that is hex 3/4/6/8 (including alpha-bearing `#rrggbbaa` from the alpha-enabled `ColorPicker` surfaces) or a CSS custom-property reference (`var(--wp--preset--color--…)` from `PanelColorSettings` theme-palette swatches, optionally with a hex fallback as `var(--token, #hex)`); for typography presets it is a `var(--wp--preset--…)` reference. Empty/null falls back to a hardcoded default in CSS. The render path runs every colour value through `Rendering\Color_Sanitizer::sanitize()` before emitting it as a CSS custom property — see [`security.md`](security.md) for the full accepted/rejected list.
