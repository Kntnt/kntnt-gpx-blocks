...
`supports.dimensions` enables `aspectRatio` and `minHeight`, which surface the standard core Dimensions panel for sizing. The chosen values land under `attributes.style.dimensions.*` like every other dimensions-aware core block and reach the wrapper as plain inline `aspect-ratio` / `min-height` styles via `get_block_wrapper_attributes()`. Empty fields fall back to the SCSS baseline (`aspect-ratio: 3 / 1; min-height: 30vh;`), and independently the PHP render path always emits an inline `min-height: 30vh` when `style.dimensions.minHeight` is blank or missing (issue #115).
...
Leaflet mounts directly into the block element when the consent contract permits it. The wrapper always has explicit dimensions: the editor's chosen `aspect-ratio` / `min-height` arrive inline via `get_block_wrapper_attributes()` when set; the plugin's render path always emits an inline `min-height: 30vh` when `style.dimensions.minHeight` is blank or missing (issue #115); and the SCSS baseline `aspect-ratio: 3 / 1; min-height: 30vh;` covers any path where neither of those reaches the wrapper. So the container always has well-defined dimensions at mount time. The plugin renders no consent UI of its own — the active CMP's content blocker is expected to reclaim the visual area when consent is denying. See [`consent.md`](consent.md) for the full consent contract.
...
Sizing is delegated to the standard core **Dimensions** panel exactly like GPX Map: `block.json` declares `supports.dimensions: { aspectRatio: true, minHeight: true }`, so the editor surfaces the standard aspect-ratio dropdown and min-height field with no plugin-specific Layout panel, no `aspectRatio` / `minHeight` attributes, and no custom validation regex. The SCSS baseline (`aspect-ratio: 4 / 1; min-height: 15vh;`) applies whenever both Dimensions fields are empty, and independently the PHP render path always emits an inline `min-height: 15vh` when `style.dimensions.minHeight` is blank or missing (issue #115) — same shape as GPX Map, scaled to half the baseline.
...
`supports.dimensions` enables `aspectRatio` and `minHeight`, with the same plumbing as GPX Map (see the Map's *Block supports* section above). Empty fields fall back to the SCSS baseline (`aspect-ratio: 4 / 1; min-height: 15vh;`), and independently the PHP render path always emits an inline `min-height: 15vh` when `style.dimensions.minHeight` is blank or missing (issue #115).
...
`supports.typography` enables every aspect — `fontFamily`, `fontSize`, `fontWeight`, `fontStyle`, `lineHeight`, `letterSpacing`, `textTransform`, `textDecoration` — with the standard `defaultControls` trio (Font, Size, Appearance) visible at the top of the panel and the rest reachable from the per-aspect dropdown menu. Typography reaches the SVG through ordinary CSS inheritance: `get_block_wrapper_attributes()` emits the chosen values as inline declarations on the wrapper, the SCSS rules for `.kntnt-gpx-blocks-elevation-axis-label` and `.kntnt-gpx-blocks-elevation-cursor-tooltip-text` set every font-* property to `inherit`, and the wrapper-padding model that uses `--kntnt-gpx-blocks-axis-label-y-width` / `--kntnt-gpx-blocks-axis-label-x-height` (defined in `em`) automatically resolves against the wrapper's chosen `font-size` so the reserved axis-label space grows with the typography choice. One typography setting applies to the whole block — editors who want differentiated axis vs tooltip styling wrap the block in a `core/group` and use that block's own typography controls on the inner content.
...
<div
    ...
    style="min-height: 15vh; --kntnt-gpx-blocks-line-color: #06c;"
>
...
**Layout model (issue #93, refined in issue #94).** The wrapper is `position: relative` and reserves space for the HTML axis-label overlays via `padding-left` / `padding-bottom` driven by font-relative CSS variables (`--kntnt-gpx-blocks-axis-label-y-width` and `--kntnt-gpx-blocks-axis-label-x-height`, both defaulting to a small multiple of `em`). The SVG is `position: absolute` and pinned to all four sides of the wrapper's content box so it grows to fill any editor-set `min-height` (the same #86 idiom the Map block uses). The viewBox dimensions are computed from the editor's `style.dimensions.aspectRatio` (or the SCSS baseline `4/1` when unset) and the SVG uses `preserveAspectRatio="xMidYMid meet"` so the polyline scales uniformly into the rendered box without stretching. Axis tick labels live in two sibling `<div>` overlays — one on the left strip for the y axis, one along the bottom for the x axis — so they honour the resolved CSS `font-size` from the block-level typography choice and reserve their own layout space outside the SVG plot area. Because the reserved-space variables are in `em`, a larger block `font-size` grows the reserved padding automatically and the polyline's draw area shrinks to fit, without any client-side measurement loop.
