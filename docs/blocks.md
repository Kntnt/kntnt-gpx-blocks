...
`supports.dimensions` enables `aspectRatio` and `minHeight`, with the same plumbing as GPX Map (see the Map's *Block supports* section above). Empty fields fall back to the SCSS baseline (`aspect-ratio: 4 / 1; min-height: 15vh;`), and `Rendering\Dimensions_Defaults` (issue #117) writes `style.dimensions.minHeight = '15vh'` onto the parsed block's `attrs` when both `minHeight` and `aspectRatio` are blank.
