 ...
 * every GPX Map block on the page so the editor can pin the elevation chart
 * to a specific map when the post has more than one.
 *
 * @since 1.0.0
 */
 ...
 * page. The body is a ServerSideRender preview that mirrors the frontend
 * SVG output.
 *
 * @since 1.0.0
 *
...
export const ElevationEdit = ( {
	...
	const { mapId, aspectRatio, minHeight } = attributes;
		...
		<>
			...
			<div { ...blockProps }>
				<ServerSideRender
					...
					attributes={ { mapId, aspectRatio, minHeight } }
				/>
			</div>
		</>
	);
